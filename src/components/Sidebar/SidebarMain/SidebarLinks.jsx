...
export default function SidebarLinks({ handleItemsClick, selected }) {
    return (
        <nav className="flex justify-center flex-col gap-3 p-1 flex-1 overflow-hidden hover:overflow-y-auto">
            {menuArray.map((item, index) => (
                <SidebarMainSingle item={item} key={index} handleItemsClick={handleItemsClick} selected={selected} />
            ))}
        </nav>
    )
}
